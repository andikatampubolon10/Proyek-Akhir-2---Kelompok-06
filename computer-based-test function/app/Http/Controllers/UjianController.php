...
class UjianController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Mencari data guru yang terkait dengan user yang sedang login
        ...
        // Mengambil id_kursus dari parameter query string
        $id_kursus = $request->input('id_kursus');
    
        // Pastikan kursus yang dipilih valid
        $kursus = Kursus::where('id_kursus', $id_kursus)->where('id_guru', $guru->id_guru)->first();
    
        if (!$kursus) {
            return redirect()->back()->withErrors(['error' => 'Kursus yang dipilih tidak valid.']);
        }
    
        // Mengambil ujian yang terkait dengan kursus yang dipilih
        $ujians = Ujian::where('id_kursus', $kursus->id_kursus)
            ->orderBy('tanggal_ujian', 'DESC')
            ->get();
    
        return view('Role.Guru.Course.index', compact('user', 'ujians', 'kursus'));
    }        
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'waktu_mulai' => 'required|date|after:now', // Pastikan waktu mulai lebih besar dari waktu saat ini
        ...
        try {
            ...
            $ujian = Ujian::create([
                ...
                'password_masuk' => $validated['password_masuk'],
                'password_keluar' => $validated['password_keluar'],
