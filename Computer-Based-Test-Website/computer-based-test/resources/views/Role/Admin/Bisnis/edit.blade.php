            ...
            <div class="w-full md:w-3/4 p-8">
                <form action="{{ route('Admin.Bisnis.update', $bisnis->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input name="nama" value="{{ $bisnis->nama }}" type="text" name="name" class="w-full border border-gray-400 p-2 rounded-lg" required>
                    </div>
                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Email<span class="text-red-500">*</span></label>
                        <input name="username" value="{{ $bisnis->username }}" type="text" name="username" class="w-full border border-gray-400 p-2 rounded-lg" required>
                    </div>
                    <div>
                        <label class="block text-gray-700 font-bold mb-2">Jumlah Pendapatan<span class="text-red-500">*</span></label>
                        <input name="jumlah_pendapatan" value="{{ $bisnis->jumlah_pendapatan }}" type="text" name="jumlah_pendapatan" class="w-full border border-gray-400 p-2 rounded-lg" required>
                    </div>
