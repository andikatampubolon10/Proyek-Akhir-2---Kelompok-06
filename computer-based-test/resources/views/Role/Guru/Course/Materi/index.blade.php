    ...
    <div class="flex flex-col md:flex-row">
        ...
        <div class="w-full bg-white p-6 shadow-md">
            ...
            @for ($week = 1; $week <= 26; $week++)
                <h4 class="text-lg font-semibold mb-2">Minggu {{ $week }}</h4>
                <div class="flex justify-between items-center mb-4">
                    <div class="flex-grow">
                        @foreach ($materi->where('week', $week) as $exam)
                            <div class="exam-container">
                                <h2 class="exam-title">
                                    {{ $exam->judul_materi}}</h2>
                            </div>
                            @if($exam->file) <!-- Pastikan materi memiliki file -->
                            File :
                            <a href="{{ Storage::url($exam->file) }}" class="text-blue-600 hover:underline" target="_blank">
                                {{ $exam->judul_materi }}
                            </a>
                        @else
                    ...
                    <div class="flex justify-end mb-4">
                        <a class="bg-green-500 text-white px-4 py-2 rounded-lg flex items-center"
                            href="{{ route('Guru.Materi.create', ['week' => $week]) }}">
